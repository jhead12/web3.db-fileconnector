...
router.get('/registry', requireAuth, async (req, res) => {
  try {
    ...
    return res.status(500).json({ error: 'Failed to retrieve plugin registry' });
  }
});
 ...
 */
router.post('/registry/submit', requireAuth, upload.single('plugin'), async (req, res) => {
  let extractedPath = '';
  ...
  try {
    ...
    const validationResults = {
      ...
      passed: !stderr.includes('FAILED') && stdout.includes('passed all validation checks')
    };
    ...
    if (validationResults.passed && req.query.install === 'true') {
      ...
      if (fs.existsSync(targetDir)) {
        ...
        const backupDir = path.join(PLUGINS_DIR, `${pluginId}-backup-${Date.now()}`);
...
router.post('/registry/install/:id', requireAuth, async (req, res) => {
  try {
    ...
    if (!fs.existsSync(tempPluginPath)) {
      return res.status(404).json({ error: 'Plugin not found in temporary storage' });
    }
    ...
    const passed = !stderr.includes('FAILED') && stdout.includes('passed all validation checks');
    if (!passed) {
      return res.status(400).json({ 
